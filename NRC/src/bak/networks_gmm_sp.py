...
class SpNet(BaseNetwork):
    def __init__(self, name='sp_net',
        ...
        self.fc = nn.Sequential(
              spectral_norm(
                  nn.Linear(z_dim // 2, block_chns[0] * 4 * 4, bias=True),
                  mode=use_spc_norm),
              nn.LeakyReLU(0.2, inplace=True)
            )
        ...
        self.decode_base_deform_f = BaseDecoder(
                          block_chns=block_chns,
                          use_spc_norm=use_spc_norm)
        # deform grid est.
        self.decode_deform_f = ImageHead(64, 2, use_spc_norm)        
    ...
    def forward(self, z, z_f, z_b):
        ...
        deform_latent = z_sp.view(bs, -1, 4, 4)
        deform_latent = torch.cat([
            z_f, deform_latent], dim=1)
        deform_latent = self.decode_base_deform_f(deform_latent)        
        deform_grid = self.decode_deform_f(deform_latent)
        # (B, 2, H, W) -> (B, H, W, 2)
        deform_grid = deform_grid.permute(0, 2, 3, 1)      
