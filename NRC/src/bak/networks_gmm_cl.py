...
class T_Z(nn.Module):
    def __init__(self, z_dim, Kf_0, Kf_1):
...
class BgNet(BaseNetwork):
    ...
    def forward(self, z):
        ...
        ### mask 
        ed = self.ed_head(bg_latent)

        ### logits for MI        
        # b_logits = self.encode_base(bg * ma.sigmoid())
        return bg, ma, bg * ma.sigmoid(), ed
...
class CEBMNet(BaseNetwork):
    def __init__(self, name='ebm_net',
        ...
        self.fg_model_0 = MLP(256, 10, nef)
        self.fg_model_1 = MLP(zf_dim, Kf // 10, nef)
        self.bg_model_0 = MLP(256, 10, nef)
        self.bg_model_1 = MLP(zb_dim, Kb // 10, nef)
    ...
    def forward(self, z):
        ...
        zf_logits_1 = self.fg_model_1(
            torch.cat(
                [zf[:, :self.zf_dim // 2].detach(), 
                 zf[:, self.zf_dim // 2:]],
                dim=1))
        zb_logits_0 = self.bg_model_0(zb[:, :self.zb_dim // 2])
        zb_logits_1 = self.bg_model_1(
            torch.cat(
                [zb[:, :self.zb_dim // 2].detach(), 
                 zb[:, self.zb_dim // 2:]],
                dim=1))
        ...
        score = torch.logsumexp(zf_logits_0, dim=1, keepdim=True) + \
                ...
                zs_logits
        return score, (zf_logits_0, zf_logits_1), \
                      (zb_logits_0, zb_logits_1), zs_logits
