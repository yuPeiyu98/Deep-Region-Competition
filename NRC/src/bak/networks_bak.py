...
class SameBlockLReLU(nn.Module):
    """ shape-preserving feature transformation """
    def __init__(self, in_channels, out_channels):
        super(SameBlockLReLU, self).__init__()
...
class FgNet(BaseNetwork):
    def __init__(self, name='fg_net',
        ...
        self.fc = nn.Sequential(
              nn.Linear(z_dim, block_chns[0] * 4 * 4 * 2, bias=False),
              GLU()
            )
        ...
        self.decode_im = nn.Sequential(*blocks)
        self.im_head = ImageHead(in_channels=64, out_channels=3)

        self.decode_ma = nn.Sequential(*blocks)        
    ...
    def forward(self, z):
        ...
        ### image 
        app_latent = self.decode_im(obj_latent)
        app = self.im_head(app_latent)

        ### displacement map
        ma_latent = self.decode_ma(obj_latent)
        ma = self.ma_head(ma_latent)        

        ### logits for MI
        app_and_ma = torch.cat([app, ma], dim=1)
        f_logits = self.encode_base(app_and_ma)
        f_logits = f_logits.view(bs, -1)
        
        return app, ma, f_logits, obj_latent
...
class SpNet(BaseNetwork):
    def __init__(self, name='sp_net',
        ...
        # intermediate feature transformation
        # residual homography grid est.
        blocks = []
        blocks.append(SameBlock(
            in_channels=block_chns[-1]+zobj_dim, 
            out_channels=64))
        for _ in range(0, num_res_blocks):
            block = ResBlock(64)
            blocks.append(block)
        blocks.append(ImageHead(64, 2))

        self.decode_deform = nn.Sequential(*blocks)

        # affine grid est.
        self.decode_base_aff = nn.Sequential(
                        nn.Linear(block_chns[0] * 4 * 4, 256 * 2),
                        GLU(),
                        nn.Linear(256, 128 * 2),
                        GLU(),
                        nn.Linear(128, 64 * 2),
    ...
    def forward(self, z, obj_latent):
        ...
        # aff = 1.5 * self.decode_a(affine_latent).view(bs, 2, 2)
        # trs = self.decode_t(affine_latent).view(bs, 2, 1)
        ...
        ############### deform estimation ##############
        deform_latent = self.decode_base_deform(
            z_sp.view(bs, -1, 4, 4))        
        ...
        return deform_grid, deform_grid, None # s_logits
...
class DNet_bg(BaseNetwork):
    def __init__(self, 
        ...
        self.encode_base = nn.Sequential(
            ...
            DownBlock(64, 128),
            # shape-preserving
            SameBlockLReLU(128, 256),
            SameBlockLReLU(256, 512),            
        ) 
