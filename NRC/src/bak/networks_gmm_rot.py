...
class SpNet(BaseNetwork):
    ...
    def forward(self, z, z_f, z_b):
        ...
        # aff params
        aff_logits = self.decode_aff(aff_latent).view(bs, 6, -1)
        aff_conf = aff_logits[:,-1:,:].softmax(dim=-1)
        aff_rot = aff_logits[:,:1,:].tanh() * np.pi / 2.
        aff_scl = aff_logits[:,1:3,:].sigmoid() * .5 + .5
        aff_trs = aff_logits[:,3:-1,:].tanh() * .2

        aff_rot_ws = (aff_conf * aff_rot).sum(dim=-1)
        aff_scl_ws = (aff_conf * aff_scl).sum(dim=-1)
        aff_trs_ws = (aff_conf * aff_trs).sum(dim=-1, keepdim=True)
        ### aff matrix
        # rotation
        cos_rot = aff_rot_ws.cos()
        sin_rot = aff_rot_ws.sin()
        theta_rot = torch.cat([cos_rot, sin_rot,
                              -sin_rot, cos_rot], dim=-1).view(bs, 2, 2)

        # scaling
        theta_scl = torch.zeros(bs, 2, 2).to(z.device)
        theta_scl[:, 0, 0] += aff_scl_ws[:, 0]
        theta_scl[:, 1, 1] += aff_scl_ws[:, 1]

        theta = torch.bmm(theta_scl, theta_rot)
        theta_inv = torch.inverse(theta)
        aff_trs_ws_inv = -torch.bmm(theta_inv, aff_trs_ws)

        rt_inv = torch.cat([theta_inv, aff_trs_ws_inv], dim=-1)
        # aff grid
        out_shape = (bs, 3, self.im_size, self.im_size)
        aff_grid = F.affine_grid(rt_inv, torch.Size(out_shape))
