...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        # channel att.
        x_avg = F.avg_pool2d(x, kernel_size=h)
        x_max = F.max_pool2d(x, kernel_size=h)

        x_mix = self.ch_model(x_avg) + self.ch_model(x_max)
        x_ch_att = x_mix.sigmoid()
        x = torch.mul(x, x_ch_att)

        # spatial att.
        x_avg = x.mean(dim=1, keepdim=True)
        x_max, __ = x.max(dim=1, keepdim=True)

        x_mix = torch.cat([x_avg, x_max], dim=1)
        x_mix = self.sp_model(x_mix)
        x_sp_att = x_mix.sigmoid()
        x = torch.mul(x, x_sp_att)
...
class BaseDecoder(nn.Module):
    def __init__(self, block_chns=[128, 1024, 512, 256, 128, 64], 
        ...
        for i in range(0, len(block_chns)-1):
            if use_att:
                block = UpAttBlock(in_channels=block_chns[i], 
