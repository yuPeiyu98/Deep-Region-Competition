...
from .networks import FgNet, BgNet, SpNet, CEBMNet, DNet_bg, DNet_all
...
class EABPModel(BaseModel):
    def __init__(self, config):
        ...
        if len(config.GPU) > 1:
            self.fg_net = nn.DataParallel(self.fg_net, config.GPU)
            self.bg_net = nn.DataParallel(self.bg_net, config.GPU)
            self.sp_net = nn.DataParallel(self.sp_net, config.GPU)
            self.ebm_net = nn.DataParallel(self.ebm_net, config.GPU)
    ...
    def _warp(self, deform_grid, aff_grid, input_im):
        ...
        return F.grid_sample(input_im, deform_grid)
    ...
    def forward(self, z, im_t):
        ...
        fg_app_cano, fg_mask_cano, f_logits, obj_latent = self.fg_net(zf)
        ...
        im_p = fg * ma + bg * (1. - ma)
        return im_p, fg, bg, ma, fg_app_cano, fg_mask_cano, \
    ...
    def sample_langevin_posterior(self, im_t, index=None):
        ...
        for __ in range(infer_step):
            ...
            # joint lkhd
            length_pe = 0.0 # self.acm_loss(fg_mask_cont) 
            aff_pe = 0.0 # bias_loss(a_grid, self.b_grid)
            deform_pe = 0.0 # bias_loss(d_grid, self.b_grid) + tv_loss(d_grid)
    ...
    def update_G(self, im_p, im_t, bg, 
        ...
        # joint lkhd
        length_pe = torch.zeros(1).to(bg.device) # self.acm_loss(fg_mask_cont) 
        ...
        # fake_bg, fake_bg_cls = self.d_net_bg(bg)
        # real_labels = torch.ones_like(fake_bg).to(fake_bg.device)
        bg_adv_loss = torch.zeros(1).cuda() # self.bEnt(fake_bg, real_labels)
        bg_adv_cls_loss = torch.zeros(1).cuda() # self.bEnt(fake_bg_cls, real_labels)
        # image adv loss
        # fake_p = self.d_net_all(im_p)
        # real_labels = torch.ones_like(fake_p).to(fake_p.device)
        ...
        G_loss = bg_adv_loss * .1 + bg_adv_cls_loss * .1 + p_adv_loss * .1 + \
                 length_pe * .1 + aff_pe * 1. + deform_pe * 1. + \
                 recon_loss + fmi * .01 + bmi * .01 + smi * .01
    ...
    def update_D(self, im_p, im_t, bg, ma):
        ...
        ma = F.interpolate(ma.detach(), size=errD_real_bg.shape[-2:])
    ...
    def learn(self, zp, zn, im_t):
        ...
        # errD_real_bg_cls = self.update_D(im_p, im_t, bg, ma)
