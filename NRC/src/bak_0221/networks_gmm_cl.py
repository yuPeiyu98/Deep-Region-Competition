...
class FgNet(BaseNetwork):
    def __init__(self, name='fg_net',
        ...
        self.fc = nn.Sequential(
                      spectral_norm(
                          nn.Linear(z_dim, 128 * 4 * 4, bias=True),
                          mode=use_spc_norm),
                      nn.LeakyReLU(0.2, inplace=True))                  
...
class CEBMNet(BaseNetwork):
    def __init__(self, name='ebm_net',
        ...
        self.fg_model = MLP(zf_dim, 200,
                            nef=nef, 
                            hi_layers=1, 
                            use_spc_norm=use_spc_norm)        

        self.bg_model = MLP(zb_dim, 200,
                            nef=nef, 
                            hi_layers=1, 
                            use_spc_norm=use_spc_norm)        
    ...
    def forward(self, z):
        ...
        score = zf_logits.logsumexp(dim=-1, keepdim=True) + \
                zb_logits.logsumexp(dim=-1, keepdim=True)
