...
class BaseNetwork(nn.Module):        
    ...
    # def init_weights(self, init_type='xavier', gain=.2):
...
class T_Z(BaseNetwork):
    def __init__(self, name='t_net',
                       z_dim=256, 
                       Ks=[2,4,5,5],                       
                       use_spc_norm=False,
                       init_weights=True):
        super(T_Z, self).__init__(name=name)
...
class FENet(BaseNetwork):
    def __init__(self, 
                 ...
                 use_spc_norm=False,
                 init_weights=True):
        super(FENet, self).__init__(name=name)
    ...
    def forward(self, x):
        ...
        f = self.enc(x).view(bs, -1)
        zs = [mlp(f) for mlp in self.mlps]
        mu = [out_mu(z) for out_mu, z in zip(self.out_mu, zs)]
        var = [out_var(z) for out_var, z in zip(self.out_var, zs)]
        return mu, var

class BENet(BaseNetwork):
    def __init__(self, 
                 name='be_net',
                 in_channels=3, 
                 z_dim=128,
                 ...
                 use_spc_norm=False,
                 init_weights=True):
        super(BENet, self).__init__(name=name)
