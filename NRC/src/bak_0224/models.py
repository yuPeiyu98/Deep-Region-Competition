...
class EABPModel(BaseModel):
    ...
    def _warp(self, deform_grid, aff_grid, input_im):
        # warped_im = F.grid_sample(input_im, deform_grid)
        # return F.grid_sample(warped_im, aff_grid)
    ...
    def forward(self, z, im_t):
        ...
        im_p = fg * ma + bg * (1. - ma)
        return im_p, fg, bg, ma, fg_app_cano, fg_mask_cano, ma, \
    ...
    def update_G(self, im_p, im_t, fg_mask_cont, bg, 
        ...
        fmi = dce(zf_logits, f_logits)  
        # bmi = torch.zeros(1).cuda() 
        bmi = dce(zb_logits, b_logits)  
        smi = torch.zeros(1).cuda() # cce(zs_logits, s_logits)  

        # joint lkhd
        length_pe = torch.zeros(1).to(bg.device) 
        ...
        deform_pe = torch.zeros(1).to(bg.device) 
