...
from .networks_gmm_h import FgNet, BgNet, SpNet, CEBMNet, DNet_bg, DNet_all
from .networks_gmm_h import BaseEncoder, T_Z
...
class EABPModel(BaseModel):
    def __init__(self, config):
        ...
        self.fge_net = T_Z(256, use_spc_norm=False)
        self.bg_net = BgNet(z_dim=config.ZB_DIM,
                            use_spc_norm=self.use_spc_norm)
        self.bge_net = T_Z(256, use_spc_norm=False)
    ...
    def update_G(self, im_t, fg_wp, bg_wp, pi_f, pi_b, 
        ...
        fmi = dce(zf_logits[0].detach(), fg_logits[0]).mean() + \
              ...
              dce(zf_logits[2].detach(), fg_logits[2]).mean() + \
              dce(zf_logits[3].detach(), fg_logits[3]).mean()
        # bmi = torch.zeros(1).cuda() 
        bmi = dce(zb_logits[0].detach(), bg_logits[0]).mean() + \
              dce(zb_logits[1].detach(), bg_logits[1]).mean() + \
              dce(zb_logits[2].detach(), bg_logits[2]).mean() + \
              dce(zb_logits[3].detach(), bg_logits[3]).mean()
    ...
    def update_E(self, en_pos, en_neg, 
        ...
        e_reg = torch.zeros(1).cuda() # ortho_reg(self.ebm_net, en_pos.device)
